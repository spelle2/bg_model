...
function [Uc,C,Ugo,Go,IGo_DA_Ach,Unogo,NoGo,INoGo_DA_Ach,Ugpe,Gpe,Ugpi,Gpi,Ut,T,Ustn,STN,E,t,Wgc_post,Wgs_post,Wnc_post,Wns_post,r,k_reward,ChI,sw] = BG_model_function_Ach(S,Wgc,Wgs,Wnc,Wns,Correct_winner,Small_winner,Dop_tonic,noiseC,gain_drop_dop,scelta_desiderata,Dop_Phasic)
% BG_model_function_Ach -----> returns dynamical behaviour of Basal Ganglia structures and cortex
% S                                             stimulus-> Column vector of 2o4 elements Ei, 0<=Ei<=1 for i=1,2,3,4
% Wgc,Wgs,Wnc,Wns                               returns synaptic weights
% gain_drop_dop,gain_adhd                       dopamine drop in case of punishment, dop in case of reward
...
% scelta_desiderata                             the neuron marked as winner
%% tempi
...
Ns=size(Wns,2);
Wcs = 1*ones(Nc,Ns); % + 0.2*diag(ones(Nc,1));
%lateral inhibition
L0 = 1.2;   %normale 1.2
...
for k = 1:D
    ...
    if isnan(r)   %nothing r=reward or punishment or nothing.
        ...
        if k >= k_reward+klatency && k <= k_reward+klatency+kduration   %time interval of dopamine change
            ...
            delta_Dop = gain_ADHD * Dop_Phasic * r_dop ^ m;            
            
            elseif r == -1        %punishment
                    switch scelta_desiderata
                            case 1
                             gain_drop_dop = 0.5;
                             Delta = -0.;
                            case 2
                ...
                delta_Dop = gain_drop_dop * Dop_Phasic * (-r_dop ^ m); 
            end
    ...
    INoGo_DA_Ach(:,k) = (beta*max(DA,0.4))+wnchi*ChI(k)*ones(Nc,1);
...
Wgs_opt_max = W_max_go*ones(Nc,Nc);   %full
Wgs_opt_min = W_min_go*ones(Nc,Nc);
Wns_opt_max = W_max_nogo*ones(Nc,Nc);   %full
Wns_opt_min = W_min_nogo*ones(Nc,Nc);
